...
jQuery(document).ready(function($){
	...
	$(".accordion.wide").on("accordionactivate", function(event, ui){
		...
		{
			$("html, body").animate({scrollTop: $("#"+$(ui.newHeader).attr("id")).offset().top-(parseInt(window.getComputedStyle(document.documentElement).getPropertyValue("--ql-bar-height"), 10) || 0)}, 400);
		}
	});
	$(".tabs").on("tabsbeforeactivate", function(event, ui){
		$("html, body").animate({scrollTop: $("#"+$(ui.newTab).children("a").attr("id")).offset().top-(parseInt(window.getComputedStyle(document.documentElement).getPropertyValue("--ql-bar-height"), 10) || 0)}, 400);
	});
	...
	$(window).on("hashchange", function(event){
		...
		else if(location.hash.substr(1,15)=="gallery-details")
		{
			...
			if(!visible)
			{
				...
				$("html, body").animate({scrollTop: galleryItemDetailsOffset.top-10-(parseInt(window.getComputedStyle(document.documentElement).getPropertyValue("--ql-bar-height"), 10) || 0)}, 400);
			}
			else
			{
				/*$(".gallery_item_details_list").animate({height:'0'},{duration:200,easing:'easeOutQuint',complete:function() 
	...
	$(".single .comments_number").on("click", function(event){
		...
		$("html, body").animate({scrollTop: offset.top-10-(parseInt(window.getComputedStyle(document.documentElement).getPropertyValue("--ql-bar-height"), 10) || 0)}, 400);
	});
